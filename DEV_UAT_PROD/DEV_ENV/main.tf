module "azurerm_resource_group" {     #RG
  source          = "../DEV_UAT_PROD/Project-A_Infra_modules/azurerm_resource_group"
  resource_groups = var.resource_group
}

module "azurerm_key_vault" {
  depends_on = [module.azurerm_resource_group]
  source     = "../DEV_UAT_PROD/Project-A_Infra_modules/azurerm_key_vaults"
  key_vaults = var.key_vault
}

module "azurerm_vnet" {
  depends_on = [module.azurerm_resource_group]
  source     = "../DEV_UAT_PROD/Project-A_Infra_modules/azurerm_virtualnetwork"
  vnets      = var.vnet
}

module "azurerm_subnet" {
  depends_on = [module.azurerm_vnet]
  source     = "../DEV_UAT_PROD/Project-A_Infra_modules/azurerm_subnets"
  subnets    = var.subnet
}

module "azurerm_ms_sql_server" {
  depends_on  = [module.azurerm_key_vault, module.azurerm_resource_group]
  source      = "../DEV_UAT_PROD/Project-A_Infra_modules/azurerm_sql_server"
...
module "azurerm_sql_server_database" {
  ...
  source        = "../DEV_UAT_PROD/Project-A_Infra_modules/azurerm_sql_database"
  sql_databases = var.sql_server_database
}

module "azurerm_nsg" {
  depends_on = [module.azurerm_subnet, module.azurerm_resource_group]
  source     = "../DEV_UAT_PROD/Project-A_Infra_modules/azurerm_nsg"
}
...
module "azurerm_subnet_nsg_asso" {
  ...
  source     = "../DEV_UAT_PROD/Project-A_Infra_modules/azurerm_subnsg_association"
}

module "azurerm_public_ip" {
  depends_on = [module.azurerm_resource_group]
  source     = "../DEV_UAT_PROD/Project-A_Infra_modules/azurerm_public_ips"
  pips       = var.pip
}
...
module "azurerm_virtual_machine" {
  ...
  source     = "../DEV_UAT_PROD/Project-A_Infra_modules/azurerm_virtual_machines"
  vms        = var.vm
}